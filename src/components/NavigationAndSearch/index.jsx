...
const NavigationAndSearch = ({ goFront, goBack, scrolled, scrolledUp, isPlaying, activeSong }) => {
  ...
  return (
  <div style={style} className={`flex flex-row items-center justify-between gap-5 z-10 top-0 left-0 w-full lg:w-[calc(100%-5px)] absolute transition-[background-color,transform] ${scrolled ? 'lg:bg-black backdrop-blur-lg' : 'bg-transparent'} ${scrollY.scrolled} ${scrolledUp && 'translate-y-[-100%] lg:translate-y-[0]'}`}>
