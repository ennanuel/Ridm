...
const Prompt = () => {
  ...
  return (
    <section className={`fixed z-[100] top-0 left-0 w-full h-full bg-black/50 backdrop-blur-sm flex items-center justify-center transition-opacity ${!displayPrompt && 'pointer-events-none opacity-0'}`}>
      <div className={`w-full max-w-[300px] overflow-hidden bg-black border border-white/10 rounded-lg transition-transform ${!displayPrompt && 'scale-50'}`}>
        <p className="p-5 font-semibold text-center text-gray-200 text-md">{promptMessage}</p>
        <div className="w-full flex text-gray-200 text-sm">
            <button 
            ...
            <button 
              className="py-2 flex-1 border-l border-white/10 bg-red-800 hover:bg-red-600" 
