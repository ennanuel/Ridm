...
const Controls = ({ dispatch, isPlaying , repeat, prev, next, pause, play, onRepeat, offRepeat, onShuffle, offShuffle, shuffle, currentIndex }) => {
  return (
    <div className="flex flex-row items-center justify-center gap-4">
        ...
        <button onClick={() => isPlaying ? pause(dispatch) : play(dispatch)} className="play_btn flex items-center justify-center bg-white/20 text-white w-[50px] h-[50px] rounded-full transition-transform active:scale-90">
            {
                isPlaying ?
                <BsPauseFill size={30} /> :
                <BsPlayFill size={30} />
