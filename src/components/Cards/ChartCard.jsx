...
const ChartCard = ({ active, type, category, i, handleClick }) => {
    ...
    return (
        <div 
            style={{'--delay': i/10 + 's'}}
            className={`group relative overflow-hidden border-2 px-4 py-2 h-full cursor-pointer rounded-xl flex flex-col items-end justify-end bg-white/5 hover:bg-black/30 transition-[opacity,transform] duration-300 border-transparent ${active ? 'opacity-0 scale-[2] border-transparent' : 'hover:border-white/5 shaodw-lg shadow-black/30'} ${origin}`} 
