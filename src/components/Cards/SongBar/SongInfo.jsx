  ...
  return (
    <div className="flex-1 flex flex-col justify-center mx-3">
      <Link to={`/songs/${song.id}`}>
        <p className="text-xs md:text-sm font-semibold text-white truncate max-w-[70%]">{song?.title}</p>
      </Link>
        <div className="flex flex-row flex-wrap gap-1 items-center text-gray-300">
          {
            ...
            <Link to={`/artists/${song.artist.id}`}>
              <p className="text-[0.75em] font-semibold text-gray-300 flex flex-row items-center justify-start gap-1 truncate">
                {song?.artist.name}
              </p>
            </Link>
          }
